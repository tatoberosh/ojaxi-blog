...
<div style="height:400px; width:100%;">
  <canvas id="snowPlayBenefitsChart"></canvas>
</div>
...
<div style="height:400px; width:100%;">
  <canvas id="snowActivityRadarChart"></canvas>
</div>
...
1.  **Dress for Success: The Art of Layering:** The key to staying warm and happy is layers. Start with a moisture-wicking base layer (like thermal underwear), add a warm middle layer (like fleece), and finish with a waterproof outer layer (snow pants and a jacket). Don't forget waterproof gloves, a warm hat, and snow boots!
2.  **Safety First:** Before sending them out, do a quick check. Ensure there are no hidden hazards under the snow (like sharp rocks or garden tools). Set clear boundaries for where it's safe to play.
3.  **Fuel the Fun:** Playing in the snow burns a lot of energy. Have warm drinks like hot chocolate and healthy snacks ready for when they come inside to refuel.
...
document.addEventListener('DOMContentLoaded', function() {
  new Chart(document.getElementById('snowPlayBenefitsChart'), {
    type: 'bar',
    data: {
      labels: ['Building a Snowman/Fort', 'Snowball Fights', 'Making Snow Angels', 'Sledding'],
      datasets: [{
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: 'Primary Benefits of Snow Activities (Rated 1-10)',
          font: { size: 16 }
        }
      },
      scales: {
        ...
        y: {
          stacked: true,
          beginAtZero: true,
          title: { display: true, text: 'Cumulative Benefit Score' }
        }
      }
    }
  });

  new Chart(document.getElementById('snowActivityRadarChart'), {
    type: 'radar',
    data: {
      labels: ['Gross Motor', 'Fine Motor', 'Creativity', 'Social Skills', 'Risk Assessment', 'STEM Skills'],
      datasets: [{
        label: 'Snowman Building',
        data: [7, 8, 9, 8, 6, 9],
        fill: true,
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: 'Developmental Profile of Snow Activities',
          font: { size: 16 }
        }
      },
      elements: {
